...
class WPBakeryVisualComposerTemplateMenuButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output($post = null) {
        if(empty($this->params)) return '';
        $output = '<li class="wpb_template_li"><a data-template_id="'.$this->id.'" href="#">'.__($this->params['name'], "js_composer").'</a> <span class="wpb_remove_template"><i class="icon-trash wpb_template_delete_icon">Delete</i> </span></li>';
        return $output;
    }
}
...
class WPBakeryVisualComposerNavBar implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function getContentLayouts() {
        ...
        foreach (WPBMap::getShortCodes() as $sc_base => $el) {
            ...
            if(!isset($settings[$role]['shortcodes'])
                || ( isset($settings[$role]['shortcodes'][$sc_base]) && (int)$settings[$role]['shortcodes'][$sc_base] == 1 ) ) {
    ...
    public function output($post = null) {
        ...
        <script type="text/javascript">
          var mk_pc_loading = "'. THEME_ADMIN_ASSETS_URI.'";
         </script>
        <div class="wpb_global_wrapper">
                ...
                <div class="navbar-inner">
                    <div class="container">
                        <div class="nav-collapse">
                            <ul class="nav">
                                <li class="dropdown">
                                    <a data-toggle="dropdown" class="dropdown-toggle wpb_popular_layouts" href="#">'.__("Columns", "js_composer").'</a>
                                    <ul class="dropdown-menu mk-columns-nav">
                                        '.$this->getColumnLayouts().'
                                    </ul>
                                </li>
                                <li class="dropdown">
                                    <a data-toggle="dropdown" class="dropdown-toggle wpb_content_elements" href="#">'.__("Elements", "js_composer").'</a>
                                    <ul class="dropdown-menu shortcode_elements">
                            ...
                            <ul class="nav">
                                <li class="dropdown">
                                    <a data-toggle="dropdown" class="dropdown-toggle wpb_templates" href="#">'.__('Templates', 'js_composer').'</a>
...
class WPBakeryVisualComposerLayout implements  WPBakeryVisualComposerTemplateInterface {
	...
	public function getContainerHelper() {
		return '<div class="container-helper">' . __('<h2>No content yet! You should add some...</h2>', 'js_composer') . __("<p>Use the buttons under <a href='javascript:open_elements_dropdown();' class='open-dropdown-content-element'>Content Elements</a> on the top or add <a href='#' class='add-text-block-to-content'>Text block</a> with single click.", 'js_composer') . '</p></div>';
	}
    ...
    public function output($post = null) {
        ...
        $output .= '<div class="metabox-composer-content">
					...
					<div id="visual_composer_content" class="wpb_main_sortable main_wrapper row-fluid wpb_sortable_container">
						<div style="margin-left:50px;"><img class="ajax-loading" src="'.THEME_ADMIN_ASSETS_URI.'/images/mk-loading.gif" /> '.__("Loading, please wait...", "js_composer").'</div>
					</div>
				...
				</div><div id="container-helper-block" style="">' . $this->getContainerHelper() . '</div></div>';
