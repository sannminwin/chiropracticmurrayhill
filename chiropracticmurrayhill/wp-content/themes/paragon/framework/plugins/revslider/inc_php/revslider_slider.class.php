	...
	class RevSlider extends UniteElementsBaseRev{
		...
		public function initByID($sliderID){
			...
			if(UniteFunctionsWPRev::isMultisite()){
				if($sliderData["siteid"] != UniteFunctionsWPRev::getBlogID())
					 UniteFunctionsRev::throwError("This slider is not permitted to view on this blog.");
			}
		...
		private function createUpdateSliderFromOptions($options,$sliderID = null){
			...
			//handle multisite:
			if(UniteFunctionsWPRev::isMultisite())
				$arrData["siteid"] = UniteFunctionsWPRev::getBlogID();
