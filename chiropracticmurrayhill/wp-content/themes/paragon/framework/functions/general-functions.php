...
*/
function mk_facebook_api_connect($username, $field, $force= false, $expiration = 3600 ) {
    $transient = 'facebook_'.$field.'_' . $username;
    $url = 'http://graph.facebook.com/'. urlencode( $username );
...
* Connets Twitter API
*/
function mk_twitter_api_connect($username, $field, $force= false, $expiration = 3600 ) {
    $transient = 'twitter_'.$field.'_' . $username;
...
/*
* Connets Dribbble API
*/
function mk_dribbble_api_connect($username, $field, $force= false, $expiration = 3600 ) {
    $transient = 'dribbble_'.$field.'_' . $username;
   $url = 'http://api.dribbble.com/'.urlencode( $username );
    $value = get_transient( $transient );
    if ( $force|| !is_numeric( $value ) || '0' == $value ) {
        $content = wp_remote_retrieve_body( wp_remote_request( $url ) );
...
function add_sumtips_admin_bar_link() {
    ...
    'title' => __( 'Masterkey Settings', 'backend'),
...
function my_show_extra_profile_fields( $user ) { ?>
    ...
    <table class="form-table">
        ...
        <tr>
         ...
         <tr>
            <th><label for="linked_in">LinkedIn</label></th>
...
/*
 * add a span element for comment author link.
 */
 ...
 */
function theme_image_resize($img_url, $width, $height, $crop = true, $retina = false, $quality = 100) {
