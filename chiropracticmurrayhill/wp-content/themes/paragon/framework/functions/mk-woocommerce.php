...
function mk_woocommerce_output_content_wrapper() {
	$page_layout = get_post_meta(get_queried_object_id(), '_layout', true);
  global $woocommerce;
	...
	function woocommerce_get_product_thumbnail( $size = 'shop_catalog', $placeholder_width = 0, $placeholder_height = 0  ) {
		global $post, $woocommerce;
			...
			if ( has_post_thumbnail() ) {
				...
				$image = theme_image_resize($image_src_array[0],  500 , 500 , true, false );
...
function mk_short_desc_filter($content){   
...
add_filter('woocommerce_short_description', 'mk_short_desc_filter');
