(function($){
    $.fn.extend({
        photostream_widget: function(options) {
			...
			jQuery('.mk-pinterest-lightbox').colorbox({
				opacity:0.7,
				maxWidth:"95%",
				maxHeight:"90%",
				current:"",
				rel: container.attr("id"),
            ...
            return this.each(function() {
				  ...
				  if (o.social_network == "dribbble") {
					  ...
					  $.getJSON("http://dribbble.com/" + o.user + "/shots.json?callback=?", function(data){
							$.each(data.shots, function(i,shot){
								...
								// .html("<div class='dribbble-widget-overlay'></h4>"
								 	//"<h4 class='dribbble-title'>" + img_title + "</h4>"
								  	//"<div class='dribbble-count'>"+shot.views_count+"</div>" +
								  	//"<div class='dribbble-author'>"+shot.player.name+"</div>" +
								  	//"<div class='dribbble-likes'>"+shot.likes_count+"</div>"
								  	//);

								  var url2 = $('<span/>').append(image);
									var final_html = $(url).after(url2);
									var li = $('<li/>').append(final_html);
								  $("ul", obj).append(li);
								}
							});
				  ...
				  if (o.social_network == "flickr") { 
						obj.append("<ul></ul>")
						$.getJSON("http://api.flickr.com/services/rest/?method=flickr.people.findByUsername&username=" + o.user+ "&format=json&api_key=bede5717cb98d3b6cdb421e4f1186fd3&jsoncallback=?", function(data){
							var nsid = data.user.nsid;
							$.getJSON("http://api.flickr.com/services/rest/?method=flickr.photos.search&user_id=" + nsid + "&format=json&api_key=bede5717cb98d3b6cdb421e4f1186fd3&per_page=" + o.limit + "&page=1&extras=url_sq&jsoncallback=?", function(data){
...
(function($){
    $.fn.extend({
        photostream_post: function(options) {
						...
						$.getJSON(url, function(data){
							$.each(data.data, function(i,shot){
								  ...
								  if (instagram_username == o.user){
									...
									if (user_id != ""){	
										url =  "https://api.instagram.com/v1/users/" + user_id + "/media/recent/?access_token=" + o.token + "&count=" + o.limit + "&callback=?";
										$.getJSON(url, function(data){
